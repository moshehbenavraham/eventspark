...
const LOGO_URLS = [
  { initials: "SL", name: "Slack" },
  { initials: "ZM", name: "Zoom" },
  { initials: "HS", name: "HubSpot" },
  { initials: "MC", name: "Mailchimp" },
  { initials: "CA", name: "Calendar" },
  { initials: "ST", name: "Stripe" },
];
      ...
      {LOGO_URLS.map((logo, i) => {
        ...
        return (
          <div key={i} className="absolute w-14 h-14 rounded-xl bg-white/80 shadow-md flex items-center justify-center" style={{ left: `calc(50% + ${Math.cos(angle) * r}px - 28px)`, top: `calc(50% + ${Math.sin(angle) * r}px - 28px)` }}>
            <span aria-label={logo.name} className="text-xs font-bold text-foreground">{logo.initials}</span>
          </div>
        );
      })}
    </div>
